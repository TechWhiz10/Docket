                ...
                <table class="w-full table-auto">
                    <thead>
                        <tr class="bg-gray text-left">
                            ...
                            @if (Auth::user()->role == config('constants.USER_ROLE_STAFF_C_LEVEL') ||
                                    Auth::user()->role == config('constants.USER_ROLE_STAFF_MANAGEMENT'))
                                <th class="py-4 px-4 font-medium text-black dark:text-white">
                                    Actions
                                </th>
                            @endif
                    ...
                    <tbody>
                        @foreach ($users as $key => $user)
                            <tr>
                                ...
                                <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    @if ($user->role == config('constants.USER_ROLE_STAFF_DEPARTMENT_MANAGER'))
                                        Department Manager
                                    @elseif ($user->role == config('constants.USER_ROLE_STAFF_TEAM_MANAGER'))
                                        Team Manager
                                    @elseif ($user->role == config('constants.USER_ROLE_STAFF_STAFF'))
                                        Staff
                                    @elseif ($user->role == config('constants.USER_ROLE_STAFF_C_LEVEL'))
                                        C-Level
                                    @elseif ($user->role == config('constants.USER_ROLE_STAFF_MANAGEMENT'))
                                        Staff Manager
                                    @endif
                                </td>
                                @if (Auth::user()->role == config('constants.USER_ROLE_STAFF_C_LEVEL') || Auth::user()->role == config('constants.USER_ROLE_STAFF_MANAGEMENT'))
                                    <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <div class="flex items-center space-x-3.5">
                                            <button wire:click="edit({{ $user->id }})" class="hover:text-primary">
